...
public class IssueBook extends javax.swing.JFrame {
    ...
    private void btnissueActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnissueActionPerformed
        if(txtid.getText().equals("") || txtstudentid.getText().equals("") || txtduedate.getText().equals("")) {
            JOptionPane.showMessageDialog(rootPane, "Please fill all the fields properly!");
            return;
        } else {
            // Check if the Book ID exists in the database
            try {
                pst = c.prepareStatement("SELECT * FROM library.book WHERE book_id = ?");
                pst.setString(1, txtid.getText());
                ResultSet rs = pst.executeQuery();

                if (!rs.next()) {  // If book ID is not found
                ...
                // Proceed with issuing the book if everything is valid
                pst = c.prepareStatement("UPDATE library.book SET status = 'Issued', issue = ?, due = ?, studentid = ? WHERE (book_id = ?)");
